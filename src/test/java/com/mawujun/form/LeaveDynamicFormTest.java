...
public class LeaveDynamicFormTest extends AbstractTest {
    ...
    @Test
    @Deployment(resources = "com/mawujun/form/leave.bpmn")
    public void testJavascriptFormType() throws Exception {
        ...
        //如果要想获取Task的Form Data，那就必须先启动流程实例，通过如下的方法先获取Task（运行中的），然后通过task_id获取Form Data。
        //Task hrTask = taskService.createTaskQuery().taskCandidateGroup("hr").singleResult();
//        TaskFormData taskformdata=formService.getTaskFormData(hrTask.getId());//
//        List<FormProperty> formProperties = taskformdata.getFormProperties();
//        for (FormProperty formProperty : formProperties) {
//            System.out.println("TaskFormData:"+formProperty.getId() + "，value=" + formProperty.getValue());
//        }
//        
    }
     ...
     * 读取历史变量并封装到Map中
     */
    private Map<String, Object> packageVariables(String processInstance_id) {
        Map<String, Object> historyVariables = new HashMap<String, Object>();
        List<HistoricDetail> list = historyService.createHistoricDetailQuery().processInstanceId(processInstance_id).list();
